...
$list_of_trip_ticket = $post->list_of_trip_ticket;
$approver_id = $post->approver_id;
$approved_by = $user_data->employee_id;
$approved_by_details = $driver->getEmployeeDetailsById($approved_by);
$approver_details = $driver->getEmployeeDetailsById($approver_id);

foreach($list_of_trip_ticket as $trip_ticket_no){

	$isApproved = $approver->isApproved($trip_ticket_no,$approver_id);
	if(!$isApproved){

		//count approvers for trip ticket
		...
		if($sequence_no <= $approver_count){
			...
			if($user_data->employee_id == $approver_id){ // the one who approved is the rightful approver
				...
				// approve trip ticket
				$approver->approveOnBehalfOf($trip_ticket_no,$approver_id,$approved_by);

				// needed data to send email to approver
				$data = array(
		...
		// checking for the next approver
		...
		if( ($sequence_no + 1) <= $approver_count ){
			...
			$emailToNextApprover = new Email();
		 	$emailToNextApprover->sendMailToApprover(
		 		array(
					"trip_ticket_no" 		=> $trip_ticket_no,
					"jo_no"			 		=> $trip_ticket_details->jo_no,
					"plate_no"	     		=> $trip_ticket_details->plate_no,
	...
	} // end of if(!$isApproved){
} // end of foreach($list_of_trip_ticket as $trip_ticket_no){