...
$file = "../reports/template/trip_log_template.xlsx";
...
$trip_ticket_report = $trip_ticket->getTripLogReport(
	$post->start_date,
	$post->end_date,
	$post->search_by,
	$vehicle_id
...
//	$time_out = ($row->time_out != "" ? Format::format_date_slash($row->time_out) : 'N/A');
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B'.$rowIndex, $row->trip_ticket_no)
	                              ...
	                              ->setCellValue('K'.$rowIndex, $row->status)
	                              ->setCellValue('L'.$rowIndex, $row->log_type)
	                              ->setCellValue('M'.$rowIndex, Format::format_date_slash($row->log_time) );
