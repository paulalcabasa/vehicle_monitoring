...
$file = "../reports/template/ccvu_template.xlsx";
...
$vehicle_usage_report = $vehicle->getVehicleUsageReport($post->start_date,$post->end_date,1);
...
foreach($vehicle_usage_report as $usage) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue('B'.$rowIndex, $usage->cs_no)
	                              ...
	                              ->setCellValue('G'.$rowIndex, $etr)
	                              ->setCellValue('H'.$rowIndex, $usage->trip_status)
	                              ->setCellValue('I'.$rowIndex, $time_in)
	                              ->setCellValue('J'.$rowIndex, $time_out)
	                              ->setCellValue('K'.$rowIndex, Format::formatBlank($usage->km_in))
	                              ->setCellValue('L'.$rowIndex, Format::formatBlank($usage->km_out))
	                              ->setCellValue('M'.$rowIndex, Format::formatBlank($usage->fuel_in))
	                              ->setCellValue('N'.$rowIndex, Format::formatBlank($usage->fuel_out))
	                              ->setCellValue('O'.$rowIndex, Format::formatBlank($usage->remarks_in))
	                              ->setCellValue('P'.$rowIndex, Format::formatBlank($usage->remarks_out));
...
$objPHPExcel->getActiveSheet()->getStyle('B'.$baseRow.":".'P'. ($rowIndex-1))->applyFromArray($report_border);
$objPHPExcel->getActiveSheet()->getStyle('B'.$baseRow.":".'P'. ($rowIndex-1))->getAlignment()->setWrapText(true); 
