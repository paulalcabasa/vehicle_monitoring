...
$(document).ready(function(){
	...
	$("#cbo_car_units").on("change",function(){
		...
		$("#txt_checklist_id").val($("#cbo_car_units option:selected").data("checklist_id"));
		$("#lbl_checklist_id").html("<a target='_blank' href='page.php?p=vckd&d=" + $("#cbo_car_units option:selected").data("enc_id") + "'>"+$("#txt_checklist_id").val()+"</a>");
		
		$("#lbl_checklist_condition").text($("#cbo_car_units option:selected").data("vehicle_condition"));
	...
	//	
		...
		if($(this).val()!="" && date!==undefined){
			$.ajax({
				...
				url:"ajax/get_vehicle_details2.php",
				success:function(response){
