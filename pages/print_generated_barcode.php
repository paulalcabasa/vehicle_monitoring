...
$barcode = new Barcode();
$barcode_list = $barcode->getBarcodes();
...
foreach($barcode_list as $bcode){
	$bcode = (object)$bcode;

	$label = $bcode->barcode_label;
	$left_additional = 0;

	$label_length = strlen($label);
	$add_space = 0;
	if($label_length > 10){
		$add_space = $label_length/4;
	}
	$space_left = (30 - $label_length + $add_space);
...
*/
		...
		$pdf->writeHTMLCell(0, 0, $barcode_x + $space_left, $barcode_y + 20, '<span>' . $label . '</span>', 0, 0, false, true, 'B',false);
		$pdf->write1DBarcode($bcode->barcode_value, 'C39', $barcode_x, $barcode_y, '', 18, 0.3, $style, 'M');
	 	$pdf->SetXY($x,$y);
		...
		$barcode_y += 35;
		/*$pdf->writeHTMLCell(0, 0, $barcode_x + $space_left, $barcode_y - 5, '<span>' . $assignee . '</span>', 0, 0, false, true, 'B',false);*/
		$pdf->writeHTMLCell(100, 50, $barcode_x + $space_left, $barcode_y + 20, '<span>' . $label . '</span>', 0, 0, false, true, '',true);
		$pdf->write1DBarcode($bcode->barcode_value, 'C39', $barcode_x, ($barcode_y), '', 18, 0.3, $style, 'M');	 	
		$pdf->Ln();
	...
	if($ctr == $barcodes_per_page){
		$barcode_x = 25;
		$barcode_y = 15;
		if($page < ($barcode_count/$barcodes_per_page) ){
			$pdf->AddPage();
		}
		$ctr = 0;
		$index = 1;
...
// tdis metdod has several options, check tde source code documentation for more information.
$pdf->Output('Generated Barcode-' . time() .'.pdf', 'I');
