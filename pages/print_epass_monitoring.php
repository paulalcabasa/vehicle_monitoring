...
if(isset($post->txt_report_date)){
	...
	$epass_monitoring_report = $epass->getEpassMonitoringReport(Format::format_date_only($start_date)." 00:00:00",Format::format_date_only($end_date)." 23:59:59");
	foreach($epass_monitoring_report as $report) {
		$report = (object)$report;
		$table_body .= '<tr>
							<td>'.$report->e_pass_id.'</td>
							<td>'.$report->plate_no.'</td>
							<td>'.$report->cs_no.'</td>
							<td>'.$report->classification.'</td>
							<td>'.$report->driver_name.'</td>
							<td>'.Format::format_date($report->log_time).'</td>
...
$pdf->SetAuthor('Paul');
$pdf->SetTitle('Epass Monitoring');
$pdf->SetSubject('Epass Monitoring');
...
$page_header = '<h1 style="text-align:center;">E-Pass Monitoring</h1>';
$page_sub_header = '<p style="font-size:11px;font-style:italic;">E-Pass Usage as of <strong>'.$start_date_display.'</strong> to <strong>'.$end_date_display.'</strong></p>';

$table_data = '<table border="1" style="font-size:10px;" cellpadding="5">
				 	<thead >
				 		<tr>
				 			<th  align="center" style="background-color:#DADFE1;color:#000;">E-pass No.</th>
				 			<th  align="center" style="background-color:#DADFE1;color:#000;">Plate No.</th>
				 			<th  align="center" style="background-color:#DADFE1;color:#000;" style="background-color:#DADFE1;color:#000;">CS No.</th>
...
// tdis metdod has several options, check tde source code documentation for more information.
$pdf->Output('E-pass-Monitoring-' . time() .'.pdf', 'I');
