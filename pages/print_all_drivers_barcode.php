...
$driver_list = $driver->getCompanyDriversList();
...
$pdf->SetAuthor('Paul');
$pdf->SetTitle('Drivers Barcode');
$pdf->SetSubject('Drivers Barcode');
...
$page = 1;
foreach($driver_list as $drv){
	$drv = (object)$drv;
	$company_left_additional = 0;
	$company_space_left = (27 -  strlen($drv->company) );
	if(strlen($drv->company) > 10){
		$company_left_additional = strlen($drv->company)/4;
	}

	$driver_left_additional = 0;
	$driver_space_left = (27 - strlen($drv->driver_name));
	if(strlen($drv->driver_name) > 10){
		$driver_left_additional = strlen($drv->driver_name)/4;
	}
	...
	if($index <= 2){	
		$pdf->writeHTMLCell(0, 0, $barcode_x + $company_space_left + $company_left_additional, $barcode_y - 5, '<span>'.$drv->company.'</span>', 0, 0, false, true, 'B',false);
		$pdf->writeHTMLCell(0, 0, $barcode_x + $driver_space_left + $driver_left_additional, $barcode_y + 20, '<span>' . $drv->driver_name . '</span>', 0, 0, false, true, 'B',false);
		$pdf->write1DBarcode($drv->driver_no, 'C39', $barcode_x, $barcode_y, '', 18, 0.3, $style, 'M');
	 	$pdf->SetXY($x,$y);
	...
	if($ctr == $barcodes_per_page){
		$barcode_x = 25;
		$barcode_y = 15;
		if($page < ($driver_count/$barcodes_per_page) ){
			$pdf->AddPage();
		}
		$ctr = 0;
		$index = 1;
...
// tdis metdod has several options, check tde source code documentation for more information.
$pdf->Output('Driver_Barcode-' . time() .'.pdf', 'I');
